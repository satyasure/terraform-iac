...
In this tutorial, we will deploy a three-tier application in AWS using Terraform.
...
> In this project, We have used some variables also that we will discuss later.
...
* Create route_table_public.tf file and add the below code to it
  ...
  route {
        ...
        gateway_id = "${aws_internet_gateway.workshopgateway.id}"
    }
  tags = {
  ...
  resource "aws_route_table_association" "rt1" {
    subnet_id = "${aws_subnet.workshopsubnet.id}"
    route_table_id = "${aws_route_table.route.id}"
  }
  # Associating Route Table
  resource "aws_route_table_association" "rt2" {
    subnet_id = "${aws_subnet.workshopsubnet1.id}"
    route_table_id = "${aws_route_table.route.id}"
  }
  ```
...
* Create web_sg.tf file and add the below code to it
  ...
  resource "aws_security_group" "workshopsg" {
    vpc_id = "${aws_vpc.workshop.id}"
  # Inbound Rules
...
* Create database_sg.tf file and add the below code to it
  ...
  ingress {
    ...
    to_port         = 3306
    protocol        = "tcp"
    security_groups = [aws_security_group.workshopsg.id]
  }
  egress {
    from_port   = 32768
    to_port     = 65535
...
* Create alb.tf file and add the below code to it
  ...
  resource "aws_lb_target_group_attachment" "attachment" {
    ...
    target_id        = aws_instance.workshopinstance.id
    port             = 80
  depends_on = [
    aws_instance.workshopinstance,
  ]
  }
  resource "aws_lb_target_group_attachment" "attachment" {
    target_group_arn = aws_lb_target_group.external-alb.arn
    target_id        = aws_instance.workshopinstance1.id
    port             = 80
  depends_on = [
    aws_instance.workshopinstance1,
  ]
  }
